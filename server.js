...
app.post('/api/auth/login',authLimiter,async(req,res)=>{
  try{if(!JWT_SECRET)return fail(res,503,'AUTH_NOT_CONFIGURED','API_JWT_SECRET is not configured.');const code=String(req.body?.code||'').trim();if(!sixDigitCode(code))return fail(res,400,'INVALID_CODE','Student code must contain exactly 6 digits.');const lookup=codeLookup(code);const found=await getStudentByLookup(lookup);if(!found)return fail(res,401,'INVALID_CREDENTIALS','Invalid or expired code.');const d=found.data;const expires=Number(d.expiresAt||0);const valid=d.status!=='disabled'&&d.status!=='deleted'&&expires>Date.now()&&await verifySecret(code,d.codeHash);if(!valid)return fail(res,401,'INVALID_CREDENTIALS','Invalid or expired code.');const now=Math.floor(Date.now()/1000);const token=signToken({sub:found.id,role:'student',iat:now,exp:now+ACCESS_TTL_SECONDS,jti:randomId(12)});await rtdbPath(`${encodeURIComponent(STUDENTS_PATH)}/${encodeURIComponent(found.id)}.json`,{method:'PATCH',body:{lastLoginAt:Date.now()}});return res.json({success:true,accessToken:token,expiresIn:ACCESS_TTL_SECONDS,student:{id:found.id,name:d.name||'',status:d.status||'active',expiresAt:new Date(expires).toISOString()}});}catch(e){console.error('student login',e);return fail(res,500,'LOGIN_FAILED','Login failed.');}
});
